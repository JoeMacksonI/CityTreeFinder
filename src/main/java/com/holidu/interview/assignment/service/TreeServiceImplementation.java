...
import com.holidu.interview.assignment.scheduler.CacheEvictScheduler;
...
public class TreeServiceImplementation implements TreeService{
    ...
    public  Map<String, Long> getTreeDataByCircle(Double x, Double y, Double radius) throws DataParseException, HoliduGeneralException {
        ...
        try {
            ...
            treeSet.stream()
                    .filter(getTreeInRadiusPredicate(x, y, radiusSquare))
                    .forEach(tree -> resultFiltered.put(tree.getTreeName(), resultFiltered.getOrDefault(tree.getTreeName(), 0L) + 1));
